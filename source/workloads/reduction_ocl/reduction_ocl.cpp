...
TestResult run(const ReductionArguments &arguments, Statistics &statistics, WorkloadSynchronization &synchronization, WorkloadIo &io) {
    ...
    printf("\n Variant 1 samples gathered %d , element count %lu , timePerIteration (us) %f Bandwidth (GB/s) %f \n", iterationCount, gws, timePerIteration / 1000.0, sizeInBytes / timePerIteration);
...
TestResult run2(const ReductionArguments &arguments, Statistics &statistics, WorkloadSynchronization &synchronization, WorkloadIo &io) {
    QueueProperties queueProperties = QueueProperties::create().setProfiling(true);
    Opencl opencl(queueProperties);
    cl_int retVal;
    ...
    printf("\n Variant 2 samples gathered %d , element count %lu , timePerIteration (us) %f Bandwidth (GB/s) %f \n", iterationCount, gws, timePerIteration / 1000.0, sizeInBytes / timePerIteration);
...
int main(int argc, char **argv) {
    ...
    ReductionWorkload::implementation = run2;
    auto status = workload.runFromCommandLine(argc, argv);
    /*ReductionWorkload::implementation = run2;
    status = workload.runFromCommandLine(argc, argv);*/
