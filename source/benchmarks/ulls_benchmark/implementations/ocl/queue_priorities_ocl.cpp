...
static TestResult run(const QueuePrioritiesArguments &arguments, Statistics &statistics) {
    ...
    //setup kernel time
    cl_uint kernelTime = static_cast<cl_uint>(arguments.kernelTime);
    ...
    for (auto i = 0u; i < arguments.iterations; i++) {
        ...
        ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(lowPriorityQueue, kernel, 1, nullptr, &gwsLowPriority, &lws, 0, nullptr, nullptr));
        ASSERT_CL_SUCCESS(clFlush(lowPriorityQueue));

        //now submit high priority kernel
        timer.measureStart();
        ASSERT_CL_SUCCESS(clEnqueueNDRangeKernel(highPriorityQueue, kernel, 1, nullptr, &gwsHighPriority, &lws, 0, nullptr, nullptr));
