...
class Stream3BytesRGBToYBenchmark : public StreamMemoryBenchmark {
  public:
    Stream3BytesRGBToYBenchmark(size_t bufferSize, uint8_t fillValue) : bufferX(bufferSize, fillValue), bufferLuminance(bufferSize / 4u, fillValue), deviceBufferX(this->bufferX.data(), sycl::range<1>{this->bufferX.size()}), deviceBufferLuminance(this->bufferLuminance.data(), sycl::range<1>{this->bufferLuminance.size()}) {}

    sycl::event run(sycl::queue &queue) override {
        auto commandList = [&](sycl::handler &cgh) {
            const size_t luminanceSize = this->bufferLuminance.size();
            auto x = this->deviceBufferX.template get_access<sycl::access_mode::read>(cgh);
            auto luminance = this->deviceBufferLuminance.template get_access<sycl::access_mode::discard_write>(cgh);

            cgh.parallel_for<class Stream3BytesRGBToYKernel>(
                sycl::range<1>{luminanceSize},
                [=](sycl::item<1> item) {
                    const int i = item.get_id() * 3;
                    luminance[item] = static_cast<uint8_t>(x[i] * 0.2126f + x[i + 1] * 0.7152f + x[i + 2] * 0.0722f);
                });
        };
        return queue.submit(commandList);
    }
...
class Stream3BytesAlignedRGBToYBenchmark : public StreamMemoryBenchmark {
    using uchar4 = sycl::vec<uint8_t, 4>;

  public:
    Stream3BytesAlignedRGBToYBenchmark(size_t bufferSize, uint8_t fillValue) : bufferX(bufferSize / uchar4::size(), uchar4{fillValue, fillValue, fillValue, 0u}), bufferLuminance(bufferSize / 4u, fillValue), deviceBufferX(this->bufferX.data(), sycl::range<1>{this->bufferX.size()}), deviceBufferLuminance(this->bufferLuminance.data(), sycl::range<1>{this->bufferLuminance.size()}) {}
    ...
    sycl::event run(sycl::queue &queue) override {
        auto commandList = [&](sycl::handler &cgh) {
            ...
            cgh.parallel_for<class Stream3BytesAlignedRGBToYKernel>(
                sycl::range<1>{luminanceSize},
                [=](sycl::item<1> item) {
                    luminance[item] = static_cast<uint8_t>(x[item].x() * 0.2126f + x[item].y() * 0.7152f + x[item].z() * 0.0722f);
                });
        };
        return queue.submit(commandList);
    }
...
static TestResult run(const StreamMemoryArguments &arguments, Statistics &statistics) {
    ...
    case StreamMemoryType::Stream_3BytesRGBtoY:
        benchmark = std::make_unique<Stream3BytesRGBToYBenchmark>(arguments.size, static_cast<uint8_t>(fillValue));
        break;
    case StreamMemoryType::Stream_3BytesAlignedRGBtoY:
        benchmark = std::make_unique<Stream3BytesAlignedRGBToYBenchmark>(arguments.size, static_cast<uint8_t>(fillValue));
        break;
    default:
