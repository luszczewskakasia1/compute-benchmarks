...
static TestResult run(const SharedLocalMemoryEmbargoArguments &arguments, Statistics &statistics) {
    ...
    size_t maxSlmSize = {};
    clGetDeviceInfo(opencl.device, CL_DEVICE_LOCAL_MEM_SIZE, sizeof(maxSlmSize), &maxSlmSize, nullptr);
    ...
    // Create kernel
    cl_program program{};
    if (auto result = ProgramHelperOcl::buildProgramFromSourceFile(opencl.context, opencl.device, "shared_local_memory.cl", nullptr, program); result != TestResult::Success) {
...
static RegisterTestCaseImplementation<SharedLocalMemoryEmbargo> registerTestCase(run, Api::OpenCL);
