...
static TestResult run(const KernelSubmitLinearKernelSizeArguments &arguments, Statistics &statistics) {
    ...
    // Setup
    // jeśli mamy inOrderQueue to tworzy listę komend, która będzie wykonywana po kolei (counter-based events)
    // może to trzeba jakoś zmienić, żeby zawsze było in oorder, nie wiem jeszcze
    // potrzebne do in order queue i do flag ajakiś
    ...
    for (size_t i = 0; i < arguments.iterations; ++i) {
        ...
        if (arguments.kernelBatchSize > 0 && ((i + 1) % arguments.kernelBatchSize) == 0) {
            ASSERT_ZE_RESULT_SUCCESS(zeCommandListHostSynchronize(cmdList, UINT64_MAX));
        }
    }
    ASSERT_ZE_RESULT_SUCCESS(zeCommandListHostSynchronize(cmdList, UINT64_MAX));
